...
//initializes the stack as a memory word array
mem_word theStack[stack_size];

//stack pointer
uint32 topOfStack = 0;

//program counter and pointer to first open address in user data segment
mem_addr ProgramCounter = text_min;
mem_addr data_current = data_min;
...
void load_kernel() {
    ...
    //The following statements put the stack machine opcodes
...
void push(mem_addr x) {

theStack[topOfStack] = data_seg[x];
topOfStack++;

}

void push_immediate(mem_addr x) {
    theStack[topOfStack] = x;
    topOfStack++;
}

void pop(mem_addr y) {

data_seg[y] = theStack[topOfStack - 1];
topOfStack--;
...
theStack[topOfStack - 2] += theStack[topOfStack - 1];
topOfStack--;
...
void print_string(mem_addr x) {
    ...
    while (data_seg[x] != '\\') {
        ...
        cout << (char) theStack[topOfStack - 1];
        x++;
    }
}
...
void print_stack_value() {
    ...
    cout << theStack[topOfStack - 1];

}


void initial_load() {
fstream firststream("stackCode.txt");
fstream num_stream("stackCode.txt");
string data_type;
string token2;
...
void load_text() {
    ...
    ifstream sourceCode("stackCode.txt");
    string token;
    string token2;
    opcode new_opcode;
    mem_addr new_addr;
